...
In Assembly, you own the house, so you take out the trash, fix the plumbing, you have to do pretty much everything by yourself.  
  
And if you mess up, your house will burn down (Seriously!).  
...
- `mov` to store data:
```
mov [rax], 0xDAD    ; Store value at the memory address
```
...
If you fail? Well, there will be consequences:  
...
In Assembly, it crashes or even worse, **keeps running with corrupted memory** which prone to security risks.

## The Great Power

Assembly gives you the raw power and you’re given a fully access to control your whole computer.  
...
So be careful because you’re now literally the garbage collector :) 
