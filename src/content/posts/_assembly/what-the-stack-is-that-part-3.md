...
## Goals
We’ve thrown away the plate analogies in [Part 1](https://leolwin.netlify.app/posts/what-the-stack-is-that-part-1/)  
  
We've seen how the CPU tracks logic with flags, how stack frames are formed and torn down with each function call in [Part 2](https://leolwin.netlify.app/posts/what-the-stack-is-that-part-2/)  
  
Now, it’s time to look for the real reason why many of us even care about the stack. Of course, to manipulate it, to break it and to hijack it.  
  
By the end of this post, I can confidently assure that you’ll:
...
Run the program. When it asks for the name, put 'A's more than 16 characters and we got...
...
```
[Local buffer name[16]]   - We control this  
[Saved RBP]               - Can overwrite with overflow  
[Return Address]          - Jackpot! We control EIP/RIP  
```
  
By overflowing name[], we push past the buffer, past rbp, and finally land on the return address.  
  
If we know **what value** to place here, we **can control** where the CPU jumps when the function ends.
...
We can use an A-Z wordlist to determine ([LiveOverflow](https://www.youtube.com/watch?v=iyAyN3GFM7A&list=PLhixgUqwRTjxglIswKp9mpkfPNfHkzyeN)'s way). You can use msf-pattern_create and msf-pattern_offset if you want.
...
If we treat the stack with respect, it’ll help us understand our code, our compiler, and our vulnerabilities.
  
But once we learn to manipulate, it becomes our weapon >:) 
