...
## Allocations 
In [Part 1](https://leolwin.netlify.app/posts/what-the-heap-part-1), we saw how the heap is managed chaos. We ask for memory, and the allocator carves it up, handing us a pointer.  
...
- What happens when you try to resize a memory block with `realloc`? (You might not want to miss this!)
...
This is the key difference: `malloc()` leaves **the allocated memory with whatever garbage values were there before**, while `calloc()` guarantees **the memory is filled with zeros**. This makes calloc slightly slower but safer if we need zero-initialized memory.
...
This is a critical lesson: after a `realloc`, we must always use the new pointer it returns, as the old one may now point to freed memory.
...
## Final Thought
We have been peeling back the layers of memory management. Keep experimenting and it's the best way to make these concepts stick.
  
[Part 3](https://leolwin.netlify.app/posts/what-the-heap-part-3) might be more interesting because it's a full exploit walkthrough. So, I'll meet you there.